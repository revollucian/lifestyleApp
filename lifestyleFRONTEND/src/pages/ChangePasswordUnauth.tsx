...
import { passwordChangeNonAuth } from "../utils/networking";
import { CommonActions } from '@react-navigation/native';
import { removeData } from "../utils/store";


const ChangePasswordUnauth = ({ navigation }: any) => {

    const [payload, setPayload] = useState({ email: '', password: '', new_password: '' });
    ...
    const changePassword = async () => {
        ...
        const change = await passwordChangeNonAuth(payload);
        if(change === 'changed'){
    ...
    return(
        <Window navigation={navigation} doesNotHaveToolbar>
            <Text
                style={styles.h1}>
                Change password
            </Text>
...
export default ChangePasswordUnauth